...
export class CreateProductCategoryComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.http.GET(`Category/GetForEdit/${this.settingID}`).subscribe((data) => {
      ...
      this.updateSettingsFormGroup.patchValue({
        productCategory: data.parentCategory,
      });
      this.updateSettingsFormGroup.patchValue({ id: data.parentCategoryId });

      data.subCategory.map((el) => {
        this.checkArray.value.push({
          id: new FormControl(el.subCategoryId),
  ...
  submitCreatePC() {
    ...
    this.checkArray.value.map((el) =>
      subarr.push({
        subCategoryName: el.subCategoryName.value,
        goal: el.sign.value == '1' ? el.goal.value * -1 : el.goal.value,
      })
    );
    arr.push({
      parentCategory: this.createSettingsFormGroup.value.productCategory,
      subCategory: subarr,
    });
    console.log(arr);
  ...
  submitEditPC() {
    ...
    this.checkArray.value.map((el) =>
      subarr.push({
        subCategoryId: el.id.value == '' ? 0 : el.id.value,
        subCategoryName: el.subCategoryName.value,
        goal: el.sign.value == '1' ? el.goal.value * -1 : el.goal.value,
        isDeleted: el.isDeleted.value,
      })
    );
    arr = {
      parentCategory: this.updateSettingsFormGroup.value.productCategory,
      parentCategoryId: this.updateSettingsFormGroup.value.id,
      subCategory: subarr,
    };
    console.log(arr);
  ...
  public addGroup(type) {
    if (type == 0) {
      if (
        this.createSettingsFormGroup.value.subCategoryName != '' &&
        this.createSettingsFormGroup.value.goal != ''
      ) {
        this.checkArray = this.createSettingsFormGroup.get(
          'subCategories'
        ) as FormArray;
        this.checkArray.value.push({
          ...
          goal: new FormControl(this.createSettingsFormGroup.value.goal),
          sign: new FormControl(this.createSettingsFormGroup.value.sign),
        });
      ...
      if (
        ...
        this.updateSettingsFormGroup.value.goal != ''
      ) {
        var itemToEdit = this.checkArray.value.filter(
          (r) => r.id.value == this.updateSettingsFormGroup.value.subCategoryID
        );
        if (itemToEdit.length == 0) {
          ...
          itemToEdit[0].goal = new FormControl(
            this.updateSettingsFormGroup.value.goal
          );
          itemToEdit[0].sign = new FormControl(
            this.updateSettingsFormGroup.value.sign
          );
  ...
  public removeGroup(name, value) {
    let selected = this.checkArray.value.findIndex(
  ...
  public editGroup(name, value, type) {
    ...
    if (type == 0) {
      this.createSettingsFormGroup.patchValue({
        subCategoryName: this.checkArray.value[selected].subCategoryName.value,
      });
      this.createSettingsFormGroup.patchValue({
        goal: this.checkArray.value[selected].goal.value,
      });
      this.createSettingsFormGroup.patchValue({
        sign: this.checkArray.value[selected].sign,
      });
    } else {
      this.updateSettingsFormGroup.patchValue({
        subCategoryID: this.checkArray.value[selected].id.value,
      });
      this.updateSettingsFormGroup.patchValue({
        subCategoryName: this.checkArray.value[selected].subCategoryName.value,
      });
      ...
      this.updateSettingsFormGroup.patchValue({
        sign: this.checkArray.value[selected].sign.value,
      });
    }
    if (selected != -1) {
      this.checkArray.value.splice(selected, 1);
